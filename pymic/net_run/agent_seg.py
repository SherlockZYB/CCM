...
class SegmentationAgent(NetRunAgent):
    ...
    def infer(self):
        ...
        if('net1' in list(checkpoint['model_state_dict'].keys())[0]):
            original_dict=self.net.state_dict()
            for param in self.net.state_dict():
                original_dict[param]=checkpoint['model_state_dict']['net1.'+param]
            self.net.load_state_dict(original_dict)
        else:
