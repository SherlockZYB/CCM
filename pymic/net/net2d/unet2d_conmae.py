...
class UNet2D_conmae(nn.Module):
    
    def __init__(self, params):
        super(UNet2D_conmae, self).__init__()
        self.params    = params
        self.in_chns   = self.params['in_chns']
        self.ft_chns   = self.params['feature_chns']
        self.dropout   = self.params['dropout']
        ...
        self.projector = nn.Sequential(nn.Linear(2048, 2048), nn.ReLU(), nn.Linear(2048, 1000))
        # self.projector = nn.Linear(2048, 1000)
    ...
    def forward(self, x):
        ...
        feature_tmp=self.avgpool(feat5)
        feature_tmp=torch.flatten(feature_tmp, 1)
        feature_tmp=self.projector(feature_tmp)
        feature = feature_tmp
        ...
        x_d0 = self.up5(x_d1)
        rec_output = self.out_conv(x_d0)

        return feature, rec_output
